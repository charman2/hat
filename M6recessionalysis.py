...
q = dataset['Q_mm'].values
et = dataset['Eta-cal_mm'].values
t = np.arange(len(q))
correction = q / (q+et)
exclude = dataset['P_mm']>1
merge_gap=1
...
hydrodata = rec.Hydrograph(q, t, correction=correction, verbose=False)
hydrodata.getevents(exclude, noise_threshold, merge_gap, min_length, verbose=False)
hydrodata.getdqdt(correction=correction, alpha=alpha, beta=beta, verbose=False)
def func(Q, a, b):
    return a * Q**b
